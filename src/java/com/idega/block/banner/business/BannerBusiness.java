...
public class BannerBusiness {
	...
	public static void saveBanner(int bannerID, int InstanceId, String attribute) {
		try {
			...
			BannerEntity banner = ((BannerEntityHome) IDOLookup.getHome(BannerEntity.class)).create();
			if (bannerID != -1) {
				update = true;
				banner = BannerFinder.getBanner(bannerID);
				if (banner == null) {
					banner = ((BannerEntityHome) IDOLookup.getHome(BannerEntity.class)).create();
					update = false;
				}
			}
	...
	public static int saveAd(int userID, int bannerID, int adID, String adName, String maxHits, String maxImpressions, String beginDate, String endDate, String URL, String fileID) {
		...
		try {
			...
			if (ad == null) {
				ad = ((AdEntityHome) IDOLookup.getHome(AdEntity.class)).create();
				update = false;
			}
			...
			return new Integer(ad.getPrimaryKey().toString()).intValue();
		}
	...
	public static DropdownMenu getAds(String name, BannerEntity banner, int userID) {
		...
		Collection list = BannerFinder.getAdsInBanner(banner, userID);
		if (list != null) {
			Iterator iter = list.iterator();
			while (iter.hasNext()) {
				AdEntity element = (AdEntity) iter.next();
				drp.addMenuElement(element.getPrimaryKey().toString(), element.getAdName());
			}
		}
	...
	public static AdEntity getCurrentAd(BannerEntity banner) {
		try {
			Collection adList = BannerFinder.getAds(banner);
			List tempList = new ArrayList(adList);
			...
			if (adList != null) {
				Iterator iter = adList.iterator();
				while (iter.hasNext()) {
					ad = (AdEntity) iter.next();
					remove = false;
					beginDate = null;
					endDate = null;
			...
			if (tempList != null && !tempList.isEmpty()) {
				if (tempList.size() > 1) {
					...
					return (AdEntity) tempList.get(random);
				}
				return (AdEntity) tempList.get(0);
			}
	...
	public static int getImageID(AdEntity ad) {
		try {
			if (ad != null) {
				List files = new ArrayList(BannerFinder.getFilesInAd(ad));
				if (files != null && !files.isEmpty()) {
					int random = (int) Math.round(Math.random() * (files.size() - 1));
					return ((Integer) ((ICFile) files.get(random)).getPrimaryKey()).intValue();
				}
				return -1;
			}
			return -1;
	...
	public static DropdownMenu getAdsDrop(String name, BannerEntity banner, int userID) {
		...
		if (list != null && !list.isEmpty()) {
			...
			while (iter.hasNext()) {
				...
				if (adName == null) {
					adName = ad.getPrimaryKey().toString();
				}
				if (isRelated(banner, ad)) {
					adName = "* " + adName;
				}
				drp.addMenuElement(ad.getPrimaryKey().toString(), adName);
			}
		}
